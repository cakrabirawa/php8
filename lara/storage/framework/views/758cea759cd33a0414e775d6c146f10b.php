                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                            ...
                            <template x-if="isConfigured">
                                <span>
                                    ...
                                    <span x-text="modalActions.length + ' <?php echo e(__('capabilities')); ?>'"></span>
                                </span>
                            </template>
                            <template x-if="!isConfigured">
                                <span><?php echo e(__('Not connected')); ?></span>
                            </template>
                        </p>
                    </div>
                </div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="text-sm font-medium text-amber-800 dark:text-amber-200">
                                    <?php echo e(__('AI not configured')); ?>

                                </p>
                                <p class="text-sm text-amber-700 dark:text-amber-300 mt-1">
                                    <?php echo e(__('Please configure your AI API key in')); ?>

                                    <a href="<?php echo e(route('admin.settings.index')); ?>?tab=integrations" class="underline font-medium"><?php echo e(__('Settings')); ?></a>.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <form @submit.prevent="executeCommand">
                    <div class="relative">
                        <textarea
                            ...
                            :placeholder="isConfigured ? '<?php echo e(__('Describe what you want to create or do...')); ?>' : '<?php echo e(__('AI is not configured')); ?>'"
                        ></textarea>
                            ...
                            <template x-if="voiceSupported">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        :title="isRecording ? '<?php echo e(__('Stop recording')); ?>' : '<?php echo e(__('Voice input')); ?>'"
                                    >
                    ...
                    <div class="mt-2 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400" x-show="!isRecording">
                        <p>
                            <kbd class="px-1.5 py-0.5 text-xs font-medium bg-gray-100 dark:bg-gray-700 rounded"><?php echo e(__('Cmd/Ctrl + Enter')); ?></kbd> <?php echo e(__('to send')); ?>
                ...
                <template x-if="processing">
                    <div class="mt-6 p-4 rounded-lg bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 border border-purple-200 dark:border-purple-700">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="flex-1">
                                <p class="text-sm font-medium text-purple-900 dark:text-purple-100" x-text="currentStep || '<?php echo e(__('Processing...')); ?>'"></p>
                                <p class="text-xs text-purple-600 dark:text-purple-400"><?php echo e(__('Please wait, this may take a moment')); ?></p>
                            </div>
                        </div>
                    ...
                    <div class="mt-6">
                        <div class="flex items-center justify-between gap-3 mb-3">
                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300 shrink-0">
                                <?php echo e(__('I can help you with:')); ?>
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400" x-text="result.status === 'success' ? '<?php echo e(__('Task completed successfully')); ?>' : (result.status === 'partial' ? '<?php echo e(__('Partially completed')); ?>' : '<?php echo e(__('Task failed')); ?>')"></p>
                        </div>
                    </div>
                    ...
                    <template x-if="result.completed_steps && result.completed_steps.length > 0">
                        <div class="mb-4 p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3"><?php echo e(__('What was done')); ?></p>
                            <ul class="space-y-2">
                        ...
                        <div class="mb-4 p-4 rounded-lg bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 max-h-60 overflow-y-auto">
                            ...
                            <template x-if="result.data.optimistic">
                                <div class="space-y-1 text-sm">
                                    <p class="text-gray-700 dark:text-gray-300"><span class="font-medium text-green-600"><?php echo e(__('Optimistic')); ?>:</span> <span x-text="'$' + Number(result.data.optimistic).toLocaleString()"></span></p>
                                    <p class="text-gray-700 dark:text-gray-300"><span class="font-medium text-blue-600"><?php echo e(__('Realistic')); ?>:</span> <span x-text="'$' + Number(result.data.realistic).toLocaleString()"></span></p>
                                    <p class="text-gray-700 dark:text-gray-300"><span class="font-medium text-amber-600"><?php echo e(__('Pessimistic')); ?>:</span> <span x-text="'$' + Number(result.data.pessimistic).toLocaleString()"></span></p>
                    ...
                    <template x-if="result.actions && Object.keys(result.actions).length > 0">
                        <div class="space-y-2">
                            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider"><?php echo e(__('Next steps')); ?></p>
...
function aiCommandModal() {
    return {
        ...
        initVoiceRecognition() {
            ...
            this.recognition.onerror = (event) => {
                ...
                if (event.error === 'not-allowed') {
                    this.voiceError = '<?php echo e(__("Microphone access denied. Please allow microphone access.")); ?>';
                } else if (event.error === 'no-speech') {
                    this.voiceError = '<?php echo e(__("No speech detected. Please try again.")); ?>';
                } else if (event.error !== 'aborted') {
                    this.voiceError = '<?php echo e(__("Voice recognition error. Please try again.")); ?>';
                }
            };
        ...
        async loadStatus() {
            this.loading = true;
            try {
                const response = await fetch('<?php echo e(route("admin.ai.command.status")); ?>');
        ...
        async executeCommand() {
            ...
            this.result = null;
            this.progressSteps = [];
            this.currentStep = '<?php echo e(__("Starting...")); ?>';

            try {
                const response = await fetch('<?php echo e(route("admin.ai.command.process-stream")); ?>', {
                    method: 'POST',
                    headers: {
                ...
                this.error = '<?php echo e(__("Failed to process command. Please try again.")); ?>';
            } finally {
            ...
            switch (event) {
                ...
                case 'complete':
                    if (data.success) {
                        ...
                        this.error = data.message || '<?php echo e(__("Something went wrong. Please try again.")); ?>';
                    }
                    break;

                case 'error':
                    this.error = data.message || '<?php echo e(__("An error occurred.")); ?>';
                    break;
            }
        },
...
<?php /**PATH D:\projects\php8\lara\resources\views/components/modals/ai-command.blade.php ENDPATH**/ ?>