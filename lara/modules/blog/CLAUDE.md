# CLAUDE.md — Blog Module

This file provides guidance to Claude Code (claude.ai/code) when working with the **Blog** module of Lara Dashboard.
...
php artisan test modules/Blog/tests/Feature/ExampleTest.php
...
```bash
php artisan module:enable Blog    # Enable the module
php artisan module:disable Blog   # Disable the module
php artisan module:migrate Blog   # Run module migrations
php artisan module:seed Blog      # Seed module data
```
...
```bash
php artisan module:make-crud Blog \
  --model=Post \
...
│   ├── Http/
│   │   ├── Controllers/
│   │   │   ├── BlogController.php     ← Base controller (extends app Controller)
...
│   ├── Providers/
│   │   ├── BlogServiceProvider.php     ← Main provider (policies, gates)
...
│   └── views/
│       ├── layouts/
│       │   └── app.blade.php                    ← Extend: `blog::layouts.app`
│       └── pages/
│           └── posts/
│               ├── index.blade.php
│               ├── create.blade.php
...
**4. Policies** — `PostPolicy` registered in `BlogServiceProvider::boot()` via `Gate::policy()`.
...
Route::prefix('admin/blog')
    ->middleware(['web', 'auth', 'permission:blog.dashboard'])
    ->name('admin.blog.')
    ->group(function () {
...
Named route format: `admin.blog.{resource}.{action}`
URL format: `/admin/blog/{resource}`
...
return view('blog::pages.posts.index', compact('posts'));
```

### Tailwind CSS Prefix

This module uses `prefix(blog)` in its Tailwind CSS config. Every Tailwind utility class in module views **must** be prefixed with `blog:`:

```blade
{{-- Correct — prefixed --}}
<div class="blog:py-16 blog:text-center blog:flex blog:items-center">
...
```bash
cd modules/Blog && npm install && npm run build
```
...
Permissions follow the pattern `blog.{resource}.{action}`:

```php
// In a permission migration (generated by module:make-crud)
Permission::create(['name' => 'blog.posts.index']);
...
// Gate check
abort_unless(auth()->user()->can('blog.posts.edit'), 403);
```
...
class PostController extends BlogController
{
    ...
    public function index(): View
    {
        ...
        return $this->renderViewWithBreadcrumbs('blog::pages.posts.index');
    }
...
Feature tests live in `modules/Blog/tests/Feature/`.
...
class PostControllerTest extends TestCase
{
    ...
    public function test_can_create_post(): void
    {
        ...
        $this->actingAsAdmin()
            ->post(route('admin.blog.posts.store'), $data)
            ->assertRedirect(route('admin.blog.posts.index'));

        $this->assertDatabaseHas('blog_posts', ['title' => $data['title']]);
    }
    ...
    public function test_can_update_post(): void
    {
        ...
        $this->actingAsAdmin()
            ->put(route('admin.blog.posts.update', $post), ['title' => 'Updated'])
            ->assertRedirect(route('admin.blog.posts.index'));
...
5. **Translations** — Use `__('blog::messages.key')` for module-scoped strings.
