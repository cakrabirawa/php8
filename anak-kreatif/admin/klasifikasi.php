          ...
          <?php if (mysqli_num_rows($res) > 0) : while ($r = mysqli_fetch_assoc($res)): ?>
              <tr class="border-b hover:bg-gray-50 dark:border-zinc-700 dark:hover:bg-zinc-800">
                ...
                <td class="p-2 text-center space-x-2 whitespace-nowrap font-bold">
                  <form action="<?= ADMIN_URL ?>klasifikasi-aksi" method="POST" class="inline ajax-form" data-redirect-url="<?= ADMIN_URL ?>klasifikasi?p=<?= $halaman ?>">
                    <?= csrf_token_input(); ?>
                  ...
                  <form action="<?= ADMIN_URL ?>klasifikasi-aksi" method="POST" class="inline" onsubmit="return confirm('Hapus klasifikasi ini?')">
