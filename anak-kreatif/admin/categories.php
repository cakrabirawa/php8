...
$total_res = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) AS total FROM kategori_produk"));
$total = $total_res['total'] ?? 0;
$total_hal = ceil($total / $batas);

$res = mysqli_query($conn, "SELECT * FROM kategori_produk ORDER BY id DESC LIMIT $batas OFFSET $offset");
?>
<?php include 'header.php'; ?>

<div class="w-full">
  <div class="mb-6">
    <h1 class="text-2xl font-black text-gray-800 dark:text-zinc-100">📁 Manajemen Kategori Produk</h1>
    <p class="text-gray-500 mt-1 dark:text-zinc-400">Atur kategori untuk koleksi buku di toko.</p>
  </div>

  <div class="flex flex-col justify-between">
    <div class="flex justify-between items-center mb-4">
      <div class="flex-grow">
        <input type="text" id="live-search-categories" placeholder="Cari kategori..." class="w-full max-w-sm p-2 border rounded-lg dark:bg-zinc-800 dark:border-zinc-600 dark:text-zinc-100">
      </div>
      <a href="<?= ADMIN_URL ?>categories-tambah" class="spa-trigger bg-green-500 text-white font-bold py-2 px-4 rounded-md shadow-sm hover:bg-green-600 transition flex-shrink-0">➕ Tambah Kategori Baru</a>
    </div>
      ...
      <table class="w-full text-left">
        ...
        <tbody id="table-categories-body">
          <?php if (mysqli_num_rows($res) > 0) : while ($r = mysqli_fetch_assoc($res)): ?>
            ...
            <tr>
              <td colspan="4" id="no-results-categories" class="text-center text-gray-400 py-10">Belum ada kategori.</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
    ...
    <?php if ($total_hal > 1) : ?>
      <div class="flex justify-center items-center gap-1 bg-white p-2 rounded-xl border font-bold dark:bg-zinc-900 dark:border-zinc-700">
        <?php if ($halaman > 1) : ?>
          <a href="<?= ADMIN_URL ?>categories?p=<?= $halaman - 1; ?>" class="px-2.5 py-1 border rounded bg-white text-gray-600 hover:bg-gray-50 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-700 dark:hover:bg-zinc-700">←</a>
        <?php endif; ?>
        <?php for ($i = 1; $i <= $total_hal; $i++) : ?>
          <a href="<?= ADMIN_URL ?>categories?p=<?= $i; ?>" class="px-2.5 py-1 rounded border <?= ($i === $halaman) ? 'bg-green-500 text-white border-green-500' : 'bg-white text-gray-600 hover:bg-gray-50 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-700 dark:hover:bg-zinc-700' ?>">
            <?= $i; ?>
          </a>
        <?php endfor; ?>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    initLiveSearch('live-search-categories', 'table-categories-body', 'no-results-categories');
  });
</script>