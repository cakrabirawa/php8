...
<body class="bg-gray-50 text-gray-800 text-xs flex h-screen overflow-hidden dark:bg-zinc-800 dark:text-zinc-200 transition-colors duration-300">
    ...
    <div class="p-4 border-t border-gray-50 space-y-3 bg-white dark:bg-zinc-900 dark:border-zinc-700">
      <div class="px-3 py-2 flex items-center gap-3 bg-gray-50 rounded-xl border border-gray-100 dark:bg-zinc-800 dark:border-zinc-700">
        <?php if (!empty($current_admin_avatar) && file_exists('../uploads/avatars/' . $current_admin_avatar)): ?>
          <img src="../uploads/avatars/<?= htmlspecialchars($current_admin_avatar); ?>" alt="Avatar" class="w-8 h-8 rounded-full object-cover">
        <?php else: ?>
          <div class="w-8 h-8 rounded-full bg-orange-200 flex flex-shrink-0 items-center justify-center text-orange-700 font-bold text-sm">
            <?= strtoupper(substr($_SESSION['admin_name'] ?? 'A', 0, 1)); ?>
          </div>
        <?php endif; ?>
        <div class="overflow-hidden">
          <p class="font-bold text-gray-700 truncate text-xs dark:text-zinc-200"><?= htmlspecialchars($_SESSION['admin_name'] ?? 'Admin'); ?></p>
  ...
  <main class="flex-grow flex flex-col h-screen min-w-0 bg-gray-50 relative dark:bg-zinc-900">
    ...
    <!-- Header Hamburger Mobile (Hanya tampil di HP) -->
    <div class="md:hidden bg-white p-4 shadow-sm flex items-center justify-between z-10 relative dark:bg-zinc-800">
      <h2 class="text-lg font-black text-orange-500">🎨 <?= htmlspecialchars(SITE_TITLE); ?></h2>
      <button onclick="toggleSidebar()" class="p-2 bg-gray-100 text-gray-600 rounded-lg text-lg focus:outline-none hover:bg-gray-200 transition dark:bg-zinc-700 dark:text-zinc-300">☰</button>
    </div>
